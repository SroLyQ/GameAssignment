...
{
	this->speed = speed;
	this->pauseTimeGun = 0.0f;
	this->pauseTimeImmune = 0.0f;
	this->hp = 6;
...
void Player::Update(sf::Texture* texture, float deltaTime, std::vector<sf::Texture*> gunTexture_R, State* state)
{
	if (!isDeadBool && state->getGameState() == GAME) {
		...
		if (isImmuneBool) {
			...
			if (immuneClockFloat - pauseTimeImmune > 2.2f) {
				isImmuneBool = false;
				immuneClock.restart();
				immuneClockFloat = 0.0f;
				pauseTimeImmune = 0.0f;
			}
			else if (immuneClockFloat - pauseTimeImmune < 0.5f) {
				bodyTexture.setFillColor(sf::Color(255, 255, 255, 100));
				isImmuneBool = true;
			}
			else if (immuneClockFloat - pauseTimeImmune < 1.0f) {
				bodyTexture.setFillColor(sf::Color(255, 255, 255, 255));
				isImmuneBool = true;
			}
			else if (immuneClockFloat - pauseTimeImmune < 1.5f) {
				bodyTexture.setFillColor(sf::Color(255, 255, 255, 100));
				isImmuneBool = true;
			}
			else if (immuneClockFloat - pauseTimeImmune < 2.0f) {
				bodyTexture.setFillColor(sf::Color(255, 255, 255, 255));
				isImmuneBool = true;
			}
		}
		...
		else if (gunType2ClockFloat - pauseTimeGun > 9.0f) {
			this->gunType = 0;
			gunType2Clock.restart();
			gunType2ClockFloat = 0.0f;
		}
		else if (gunType3ClockFloat - pauseTimeGun > 7.0f) {
			this->gunType = 0;
			gunType3Clock.restart();
...
void Player::setPauseTime(float pauseTime)
{
	this->pauseTimeGun += pauseTime;
	this->pauseTimeImmune += pauseTime;
}
